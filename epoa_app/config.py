...
class Config:
    ...
    def conf_file(self) -> Path:
        if ecfn := os.environ.get("EPOACONF"):
            ecf = Path(ecfn)
            if not ecf.is_file():
    ...
    @cached_property
    def dir(self) -> Path:
        return self.value_to_path("dir", HOME / "epoa").absolute()
    ...
    def spreadsheet(self) -> Path:
        return self.value_to_file_path("spreadsheet", "epoa.xlsx")
